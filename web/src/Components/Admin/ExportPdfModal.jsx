...
const ExportPdfModal = ({ isOpen, onClose, currentFilters, currentActiveTab, API_BASE_URL }) => {
  ...
  const [bungaMarketGrade, setBungaMarketGrade] = useState(currentFilters?.market_grade || '');
  const [bungaHealthClass, setBungaHealthClass] = useState(currentFilters?.health_class || '');
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (dataType === 'bunga' || dataType === 'all') {
        ...
        if (bungaMinConfidence) filters.minConfidence = bungaMinConfidence;
        if (bungaMaxConfidence) filters.maxConfidence = bungaMaxConfidence;
      }

      if (dataType === 'leaf' || dataType === 'all') {
        if (leafDisease) filters.disease = leafDisease;
        if (leafMinConfidence) filters.minConfidence = leafMinConfidence;
        if (leafMaxConfidence) filters.maxConfidence = leafMaxConfidence;
      }
      ...
      >
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {['bunga', 'leaf', 'all', 'activities'].map((type) => (
              <label key={type} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                ...
                <span style={{ color: colors.text, textTransform: 'capitalize' }}>
                  ...
                  {type === 'activities' && 'All Activities'}
                </span>
              </label>
            ))}
          </div>
